...
def combine_lectures_in_module(root_directory):
    for module in os.listdir(root_directory):
        ...
        if os.path.isdir(module_path):
            ...
            combined_file_path = os.path.join(root_directory, f"{cleaned_module_name}.txt")
            with open(combined_file_path, 'w', encoding='utf-8') as combined_file:
                combined_file.write(combined_lectures)
            print(f"Combined lectures for module {cleaned_module_name} into {combined_file_path}")


if __name__ == '__main__':
    combine_lectures_in_module(input('Enter the root directory of the lecture modules:'))
